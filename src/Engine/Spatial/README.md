...
| 커널       | 셰이더                              | 역할                                                     |
| ---------- | ----------------------------------- | -------------------------------------------------------- |
| integrate  | `advanced_tsdf_integrate.comp.glsl` | 포인트 → band 복셀 findOrInsert + 누적                   |
| compact    | `advanced_tsdf_compact.comp.glsl`   | 점유 슬롯만 core-filter + world-decode → `AdvancedEntry` |
| clear      | `advanced_tsdf_clear.comp.glsl`     | 슬롯당 1스레드로 EMPTY 초기화 (24MB 호스트 업로드 대체)  |
| **rehash** | `advanced_tsdf_rehash.comp.glsl`    | auto-grow 시 점유 슬롯을 더 큰 해시로 재삽입             |
