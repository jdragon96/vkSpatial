...
#include "Registration/RigidTransform.h"
...
namespace Registration {
    ...
    namespace {
        ...
        // not a trustworthy coarse alignment). Not part of Registration::RegistrationConfig (Task 1's
        ...
        // were inliers.
        size_t CountInliers(const Eigen::Matrix4f &T, const Common::PointCloud &src, const Common::PointCloud &tgt,
        ...
        // Estimate(). In addition to the Registration::RegistrationResult, this also hands back the
        ...
        CoarsePipelineResult RunCoarsePipeline(const Common::PointCloud &src, const Common::PointCloud &tgt,
                                                const Registration::RegistrationConfig &cfg) {
            CoarsePipelineResult out;
            Registration::RegistrationResult &result = out.result;

            out.srcDs = DownsampleVoxel(src, cfg.voxelSize);
            out.tgtDs = DownsampleVoxel(tgt, cfg.voxelSize);
            const Common::PointCloud &srcDs = out.srcDs;
            const Common::PointCloud &tgtDs = out.tgtDs;
    ...
    Registration::RegistrationResult EstimateRansac(const Common::PointCloud &src, const Common::PointCloud &tgt, const Registration::RegistrationConfig &cfg) {
        return RunCoarsePipeline(src, tgt, cfg).result;
    }

    Registration::RegistrationResult Estimate(const Common::PointCloud &src, const Common::PointCloud &tgt, const Registration::RegistrationConfig &cfg) {
        ...
        Registration::RegistrationResult refined = coarse.result;
